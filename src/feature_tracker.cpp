        ...
        std::vector<cv::Point2f> &vpriorkps, std::vector<bool> &vkpstatus) const
{
    ...
    {
        // // show vnewkps in curFrame and vkps in pkf
        // cv::Mat outkpsImg1 = slam_visualizer::drawKeyPoints(vprevpyr[0], vkps);
        // cv::Mat outkpsImg2 = slam_visualizer::drawKeyPoints(vcurpyr[0], vpriorkps);
        // cv::Mat outMatchesImg = slam_visualizer::drawMatches(vprevpyr[0], vcurpyr[0], vkps, vpriorkps, vkpstatus);

        // cv::imshow("outkpsImg1", outkpsImg1);
        // cv::imshow("outkpsImg2", outkpsImg2);
        // cv::imshow("outMatchesImg", outMatchesImg);
    ...
    {
        // // show vnewkps in curFrame and vkps in pkf
        // cv::Mat outkpsImg1 = slam_visualizer::drawKeyPoints(vprevpyr[0], vnewkps);
        // cv::Mat outkpsImg2 = slam_visualizer::drawKeyPoints(vcurpyr[0], vbackkps);
        // cv::Mat outMatchesImg = slam_visualizer::drawMatches(vprevpyr[0], vcurpyr[0], vnewkps, vbackkps);
        ...
        // // show backward track error
        // std::vector<std::pair<cv::Point2f, cv::Point2f>> correspondings;
        // for(int i=0; i<vkps.size(); ++i) {

        //     int idx = vkpsidx.at(i);
        //     correspondings.push_back({vkps[idx], vbackkps[i]});  // bug: vpks[i] vbackkps[i] 并非一一对应  应该是vpks[idx]
        // }
        // cv::Mat outReproImg = slam_visualizer::drawRepreojectError(vprevpyr[0], correspondings, vkpstatus);

        // cv::imshow("outReproImg", outReproImg);
